document.addEventListener("DOMContentLoaded", () => {
  ...
  function renderOrders() {
    ...
    let waiting = 0, ready = 0, incomplete = 0;
    ...
    activeOrders.forEach(orderId => {
      ...
      if (order.status === "waiting") timeLabel.textContent = `Waiting ${minutesSince} min`;
      if (order.status === "ready") timeLabel.textContent = `Ready ${minutesSince} min`;
      if (order.status === "incomplete") timeLabel.textContent = `Waiting ${minutesSince} min`;
  ...
  async function handleAddOrder() {
    ...
    if (!orderId) {
      searchMessage.textContent = "Please enter an order number.";
      return;
    }

    const orderRef = db.ref(`orders/Food Canteen/${orderId}`);
    const snapshot = await orderRef.get();

    if (snapshot.exists()) {
      searchMessage.textContent = "Order already exists.";
    ...
    searchMessage.textContent = "Order added.";
    orderNumberInput.value = "";
  }
